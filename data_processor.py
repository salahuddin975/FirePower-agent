...
class SummaryWriter:
    ...
    def _initialize(self):
        with open(f'{self._file_name}_v{self._model_version}.csv', 'w') as fd:
            ...
            writer.writerow(["model_version", "episode_number", "max_reached_step", "total_penalty", "load_loss", "active_line_removal"])
    ...
    def add_info(self, episode, max_reached_step, episodic_penalty, load_loss):
        with open(f'{self._file_name}_v{self._model_version}.csv', 'a') as fd:
            writer = csv.writer(fd)
            writer.writerow([str(self._model_version), str(episode), str(max_reached_step), str(episodic_penalty), str(load_loss), str(episodic_penalty-load_loss)])
